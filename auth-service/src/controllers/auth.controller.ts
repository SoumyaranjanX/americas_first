...
import { IUser, UserRole, UserStatus } from '@austa/shared/interfaces/user.interface';
...
export class AuthController {
  ...
  constructor() {
    ...
    getRedisClient().then((redisClient: RedisClientType) => {
      this.authService = new AuthService(UserModel, redisClient, auth0Client, securityMetrics);
    }).catch((error: Error) => {
