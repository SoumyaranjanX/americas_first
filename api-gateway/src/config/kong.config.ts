...
export function validateConfig(config: any): boolean {
  try {
    ...
    if (ENVIRONMENT !== 'development') {
      const allRoutes = Object.values(config.routes);
      const allRoutesUseHttps = allRoutes.every((route: any) => 
        route.protocols && route.protocols.includes('https')
      );
      if (!allRoutesUseHttps) {
  ...
  services: {
    ...
    auth: {
      ...
      health_checks: {
        ...
        passive: {
          healthy: { successes: 2 },
          unhealthy: { http_failures: 2, timeout: 10000 }
        }
      }
    },
  ...
  routes: {
    ...
    auth: {
      paths: [
        ...
        `/api/${API_VERSION}/auth/verify`,
        `/api/${API_VERSION}/auth/refresh`,
        `/api/${API_VERSION}/auth/logout`,
        `/api/${API_VERSION}/auth/password-reset`,
        `/api/${API_VERSION}/auth/mfa`,
        `/api/${API_VERSION}/auth/session`
      ],
