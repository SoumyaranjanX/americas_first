...
import { Logger, globalLogger } from '@austa/shared/middleware/logger';
...
function setupRoutes(app: express.Express): void {
  ...
  Object.entries(kongConfig.services).forEach(([serviceName, serviceConfig]) => {
    // Extract the base service path prefix (e.g., '/auth' for auth service)
    const basePathPrefix = `/${serviceName}`;
    
    console.log('🔧 Setting up proxy for service:', {
      service: serviceName,
      pathPrefix: basePathPrefix,
      targetUrl: serviceConfig.url,
      routePaths: kongConfig.routes[serviceName as ServiceName].paths
    });
    ...
    const proxyMiddleware = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
      try {
        ...
        // Calculate target path by preserving the full path after the service prefix
        const targetPath = req.originalUrl.replace(`/api/${API_VERSION}${basePathPrefix}`, '');
    ...
    apiRouter.use(basePathPrefix, proxyMiddleware);
