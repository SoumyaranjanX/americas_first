...
export const useWebRTC = (
  ...
  const monitorConnectionQuality = useCallback(async () => {
    ...
    try {
      ...
      // Process stats array to get the metrics
      stats.forEach((stat: any) => {
        if (stat.kind === 'audio') {
          audioLevel = stat.inputLevel || audioLevel;
          packetLoss += stat.packetsLost || 0;
        }
        if (stat.kind === 'video') {
          videoBitrate = stat.bitrate || videoBitrate;
          packetLoss += stat.packetsLost || 0;
        }
      });
      ...
      await virtualCareApi.reportConnectionQuality(consultationId, quality);
    } catch (err) {
  ...
  const initializeConnection = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/virtual-care/sessions/${consultationId}/join`, {
        method: 'POST',
        headers: {
      ...
      const consultationRoom = await response.json();
  ...
  const disconnect = useCallback(async () => {
    ...
    try {
      await fetch(`/api/virtual-care/sessions/${consultationId}/end`, {
        method: 'POST'
      });
