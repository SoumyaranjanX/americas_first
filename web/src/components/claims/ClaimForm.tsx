...
const claimValidationSchema = yup.object().shape({
  type: yup.mixed<ClaimType>()
    .oneOf(Object.values(ClaimType))
  ...
  documents: yup.array(yup.mixed<File>()
  ...
  hipaaAuthorization: yup.boolean()
    ...
    .required('HIPAA authorization is required'),
  consentAcknowledgment: yup.boolean()
    .oneOf([true], 'Consent acknowledgment is required')
    .required('Consent acknowledgment is required')
}) satisfies yup.ObjectSchema<IClaimSubmission>;

// Props interface with security requirements
interface ClaimFormProps {
  onSubmitSuccess: (claim: IClaim) => void;
  onSubmitError: (error: Error) => void;
  onFormChange?: (formData: IClaimSubmission) => void;
...
const ClaimForm: React.FC<ClaimFormProps> = ({
  ...
  const {
    ...
    defaultValues: {
      ...initialData,
      documents: [] // Initialize documents as empty array
    },
  ...
  useEffect(() => {
    if (onFormChange) {
      const subscription = watch((formData) => {
        ...
        const validFormData: IClaimSubmission = {
          ...formData as IClaimSubmission,
          documents: uploadedFiles
        };
        onFormChange(validFormData);
      });
      return () => subscription.unsubscribe();
    }
  }, [watch, onFormChange, uploadedFiles]);
