...
class VirtualCareApi {
  ...
  /**
   * Verifies encryption capabilities for the consultation
   */
  private async verifyEncryptionCapabilities(requirements: ConsultationCreateParams['encryptionRequirements']): Promise<void> {
    try {
      const response = await this.axiosInstance.post(
        `${VirtualCareEndpoints.GET_SESSION_TOKEN}/verify-encryption`,
        requirements
      );

      if (!response.data.isSupported) {
        throw new Error('Required encryption capabilities not supported');
      }
    } catch (error) {
