...
class AuthAPI {
  ...
  public async login(credentials: ILoginCredentials): Promise<{ token: string; fingerprint: string; user: IUser }> {
    try {
      console.log('Login URL:', buildUrl(AuthEndpoints.LOGIN));
      const response = await this.client.post(
   ...
   */
  public async sendPasswordResetEmail(params: { email: string }): Promise<void> {
    try {
      await this.client.post(
        buildUrl(AuthEndpoints.RESET_PASSWORD),
        params
      );
...
/**
 * Verifies a user's account using the provided token and user ID
 * @param token - The verification token from the email link
 * @param userId - The user's ID
 * @returns Promise that resolves when verification is successful
 * @throws Error if verification fails
 */
export const verifyAccount = async (token: string, userId: string): Promise<void> => {
