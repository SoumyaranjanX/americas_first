...
const validateHIPAACompliance = (record: Partial<IHealthRecord>): void => {
  if (!record.patientId || !record.content) {
    throw new Error('PHI validation error');
  }
...
/**
 * Health Records API client with HIPAA compliance and security measures
 */
class HealthRecordsApi {
  private async handleError(error: any, context: string): Promise<never> {
    const errorCode = error.response?.data?.code || ErrorCode.API_ERROR;
  ...
  /**
   * Fetches a health record by ID with security validation
   */
  public async getHealthRecord(recordId: string): Promise<HealthRecordResponse<IHealthRecord>> {
    try {
      const url = buildUrl(HealthRecordEndpoints.GET_RECORD);
      const response = await fetch(url, {
        headers: {
          'Content-Type': 'application/json',
          'X-Security-Context': 'HIPAA-COMPLIANT'
        }
      });
      ...
      return this.handleError(error, 'getHealthRecord');
    }
  }

  /**
   * Creates a new health record with security classification
   */
  public async createHealthRecord(record: Omit<IHealthRecord, 'id'>): Promise<HealthRecordResponse<IHealthRecord>> {
    try {
      const response = await fetch(HealthRecordEndpoints.CREATE_RECORD, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Security-Context': 'HIPAA-COMPLIANT'
      ...
      return this.handleError(error, 'createHealthRecord');
    }
  }

  /**
   * Updates an existing health record with audit logging
   */
  public async updateHealthRecord(recordId: string, updates: Partial<IHealthRecord>): Promise<HealthRecordResponse<IHealthRecord>> {
    try {
      const url = buildUrl(HealthRecordEndpoints.UPDATE_RECORD);
      const response = await fetch(url, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
      ...
      return this.handleError(error, 'updateHealthRecord');
    }
  }

  /**
   * Deletes a health record with security verification
   */
  public async deleteHealthRecord(recordId: string): Promise<HealthRecordResponse<void>> {
    try {
      const url = buildUrl(HealthRecordEndpoints.DELETE_RECORD);
      const response = await fetch(url, {
        method: 'DELETE',
        headers: {
          'X-Security-Context': 'HIPAA-COMPLIANT'
      ...
      return this.handleError(error, 'deleteHealthRecord');
    }
  }
}
