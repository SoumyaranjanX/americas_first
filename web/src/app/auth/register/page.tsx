...
import { AuthState, IUser, IAuthError } from '../../../lib/types/auth';
...
const RegistrationPage: React.FC = () => {
  ...
  const handleBiometricSuccess = useCallback(async (result: AuthResult) => {
    try {
      ...
      // The actual registration will be handled by the form

    } catch (error) {
      console.error('Biometric authentication success handling failed:', error);
    }
  }, [securityContext]);

  const handleBiometricError = useCallback((error: BiometricError) => {
  ...
  const handleRegistrationSuccess = useCallback(async (user: IUser, mfaSetup: { type: string; verified: boolean }) => {
    try {
