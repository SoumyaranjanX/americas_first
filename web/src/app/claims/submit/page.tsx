...
import { IClaim, IClaimSubmission, ClaimStatus } from '../../../lib/types/claim';
...
const ClaimsSubmitPage: React.FC = () => {
  ...
  const [complianceStatus, setComplianceStatus] = useState<{ isCompliant: boolean; violations: string[] }>({
    isCompliant: true,
    violations: []
  });

  // Handle form changes
  const handleFormChange = useCallback((formData: IClaimSubmission) => {
    // Create a temporary claim object for validation
    const tempClaim: IClaim = {
      id: '',
      version: 1,
      claimNumber: '',
      patientId: '',
    ...
    const result = validateCompliance(tempClaim);
    setComplianceStatus({
