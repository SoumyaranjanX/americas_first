...
export default function HomePage() {
  ...
  const securityContext = {
    ...
    authToken: auth.user?.securitySettings?.lastPasswordChange.toString() || '',
    ipAddress: 'masked',
    deviceId: auth.user?.securitySettings?.deviceFingerprints?.[0] || ''
  };
          ...
          <Grid item xs={12}>
            <Suspense fallback={<div>Loading health metrics...</div>}>
              <HealthMetrics
                ...
                showHistory={true}
                encryptionKey={auth.user.securitySettings?.lastPasswordChange.toString() || ''}
